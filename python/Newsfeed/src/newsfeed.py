...
feedparser.USER_AGENT = 'newsfeed/4.0'
...
def load_feed(vps, cache, line_number, feed):
    """ Main method.
        Args:
            vps: VpsUtils
            cache: cache db
    ...
    for item in rss['items']:
        ...
        if cache.get(link, '') == '':
            if not title_emitted:
                vps.html_add('<h1><a href="' + feed["WebUrl"])
                vps.html_add('">' + feed["Name"] + "</a></h1>")
                vps.html_add('<table style="width: 100%">')
                title_emitted = True

            line_number = add_news(vps, line_number, item['title'], link)
            cache[link] = cache_time

    if title_emitted:
        vps.html_add('</table><p><br /></p>')
...
def add_news(vps, line_number, title, url):
...
def main():
    ...
    for feed in feeds:
        line_number = load_feed(vps, cache, line_number, feed)
