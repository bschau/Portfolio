...
void wm_init_dock(const char *program_name, int dock_window_size, int icon_window_size, wm_cmds *cmds)
{
	...
	dock_icon_window = XCreateSimpleWindow(display, dock_window,
		(dock_window_size - icon_window_size) / 2,
		(dock_window_size - icon_window_size) / 2,
		icon_window_size, icon_window_size, 0, _black, _black);
...
void wm_event_loop(void)
{
	...
	for (EVER) {
		...
		case ButtonRelease:
			if (ev.xbutton.x < 0 ||
				ev.xbutton.x > _icon_window_size ||
				ev.xbutton.y < 0 ||
				ev.xbutton.y > _icon_window_size) {
				break;
			}
...
void wm_spawn(char *cmd)
{
	...
	if (result != 0) {
		fprintf(stderr, "Result from running '%s' = %i\n", cmd, result);
	}
