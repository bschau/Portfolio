...
namespace SecureMail
{
	public class SecureMailMessage
	{
		...
		byte[] GetSignature(string message)
		{
			...
			cmsSigner.Certificates.Add(SealCertificate);
		...
		public MailMessage ToMailMessage()
		{
			...
			mailMessage.Headers.Add("Message-ID", Guid.NewGuid() + "@schau.dk");
			...
			var contentView = new AlternateView(new MemoryStream(bytes), contentType)
			{
		...
		string NewBoundary()
		{
			return string.Format("bumblebee-{0}", Guid.NewGuid().ToString());
		}
	}
}
