#!/bin/bash
vault="/media/bs/DATA/Vault"
remote="/media/bs/SDCard"
folders="Films Pictures RebeccaYoutube"
dryrun="--dry-run"
slow=""

if test ! -d "${remote}/lost+found"; then
	echo "${remote} seems not to be mounted?" >&2
	exit 1
fi

remoteVault="${remote}/Vault"
if test ! -d "$remoteVault"; then
	echo "No Vault file on $remote" >&2
	exit 1
fi
...
for folder in $folders; do
	src="${vault}/${folder}"
	dst="${remoteVault}/${folder}"

	echo "Synchronizing $folder to $remoteVault"
