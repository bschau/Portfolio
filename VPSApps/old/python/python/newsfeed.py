...
feedparser.USER_AGENT = 'pynewsfeed/2.0'
...
def get_cache_file(vps):
    """ Get the cache file.
        Arguments:
            vps: VpsUtils
    """
    file = '_pynewsfeed' if platform.system() == 'Windows' else '.pynewsfeed'
...
def main():
    ...
    # Nyheder
    line_number = load_feed(vps, cache, line_number, 'DR',
                            'http://www.dr.dk/nyheder/service/feeds/allenyheder')
    line_number = load_feed(vps, cache, line_number, 'TV2',
                            'http://feeds.tv2.dk/nyhederne_seneste/rss')
    line_number = load_feed(vps, cache, line_number, 'Ganl&oslash;se',
                            'http://feeds.feedburner.com/GanlseByOgOmegn-Forside')
    line_number = load_feed(vps, cache, line_number, 'Egedal',
                            'http://egedal.lokalavisen.dk/section/senestenytrss')

    # Sport
    line_number = load_feed(vps, cache, line_number, 'DR Sport',
                            'http://www.dr.dk/nyheder/service/feeds/sporten')
    line_number = load_feed(vps, cache, line_number, 'TV2 Sport',
                            'http://feeds.tv2.dk/sporten_seneste/atom')

    # IT
    lurl = 'https://github.com/phw198/OutlookGoogleCalendarSync/releases.atom'
    line_number = load_feed(vps, cache, line_number,
                            'Outlook Google Calendar Sync',
                            lurl)
    line_number = load_feed(vps, cache, line_number, 'OMG Ubuntu',
                            'http://feeds.feedburner.com/d0od')
    line_number = load_feed(vps, cache, line_number, 'Mobil',
                            'http://mobil.nu/feed')
    line_number = load_feed(vps, cache, line_number, 'Mobilsiden',
                            'https://www.mobilsiden.dk/xml/rssfeed.php')
    line_number = load_feed(vps, cache, line_number, 'V2',
                            'http://www.version2.dk/it-nyheder/rss')
    line_number = load_feed(vps, cache, line_number, 'Beep',
                            'http://feeds.tv2.dk/beep_seneste/rss')

    # Blogs
    line_number = load_feed(vps, cache, line_number, 'Ayende',
                            'http://feeds.feedburner.com/AyendeRahien')
    line_number = load_feed(vps, cache, line_number, 'Benjojo',
                            'https://blog.benjojo.co.uk/rss.xml')

    # Sjov
    line_number = load_feed(vps, cache, line_number, 'DailyWTF',
