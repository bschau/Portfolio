 ...
 * Put a .htaccess into the mailqueue folder with the following content:
 *
 * 	deny from all
 *
 * Version: 1.0
...
function fcPopup(parent, id) {
	...
	el.innerHTML = '<a href="$url/download.php?f=' + id + '">Download</a> | <a href="$url/delete.php?f=' + id + '">Slet</a>';
...
EOF;
	...
	while (list($k, $file) = each($fArr)) {
		...
		echo "<tr id=\"pop" . $elem . "\"><td><a href=\"javascript:fcPopup('pop" . $elem . "', '" . $file . "')\">" . base64_decode($file) . "</td><td>" . $mTime . "</td><td>" . $size . "</td></tr>";
		$elem++;
	}
...
function FileCache_upload() {
	...
	if ((is_user_logged_in()) &&
		...
		$dst = $fc[path] . "/" . base64_encode(basename($_FILES['file']['name']));
...
<td><input type="text" name="filecache[path]" value="$fc[path]" /></td>
</tr>
</table>
<p class="submit">
